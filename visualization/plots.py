...
class VisualizeData:
    ...
    def graph_line_2_funct(self, title, xdata_input, ydata1_input, ydata2_input, color1, color2, legend1, legend2):
        """
        Create a line graph for two functions.
        """
        graph = figure(title = title, width=500, height=500, x_axis_label='X', y_axis_label='Y', background_fill_color="#fafafa")
    ...
    def graph_patch(self,title, x_test, y_test, x_colored, y_colored, x_ideal, y_ideal, x_patch, y_patch, N_y_ideal):
        """
    ...
    def visualize(self):
        ...
        graph_test_4ideals = figure(title = "Mapped points between 4 ideal functions & test data",width=500, height=500, x_axis_label='X', y_axis_label='Y', background_fill_color="#fafafa")
        ...
        for column in range(4):
            ...
            y_ideal_train_list.append(
                self.graph_line_2_funct(
                    f"Train data y{column + 1} & ideal function {N_y_ideal}", x_ideal, y_train, y_ideal, "red", "blue", f"train y{column + 1}", f"ideal {N_y_ideal}"
                )
            )
            ...
            deviation_graphs_list.append(
                self.graph_patch(f"Mapped points between ideal function {N_y_ideal} & test data",x_test, y_test, x_ideal_colored, y_ideal_colored1, 
